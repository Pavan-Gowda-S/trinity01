...
import { MetaRow, Panel, RiskPill, SectionTitle } from "@/components/trinity/ui";
...
function CaseIntelligence() {
  ...
  const { cases, log, addAlert, setAlertStatus, alerts } = useTrinity();
  ...
  const pipeline = usePipeline(() => {
    ...
    setFocusCluster(record.clusterId);
    addAlert({
      id: `ALT-${Math.floor(Math.random() * 9000 + 1000)}`,
      caseId,
      clusterId: record.clusterId,
      priority: riskLevel(record.riskScore),
      status: "New",
      createdAt: new Date().toISOString().slice(0, 16).replace("T", " "),
      note: record.lowConfidence
        ? "Insufficient evidence for reliable location prediction — manual investigation recommended."
        : `Potential cash-out activity predicted in ${cluster?.label} (${cluster?.area}).`,
    });
    log(`Prediction generated — ${cluster?.label}, risk ${record.riskScore}/100`, caseId, "Prototype Model");
    log("Alert created from prediction pipeline", caseId, "System");
    toast.success(
      record.lowConfidence
        ? "Prediction complete — low confidence"
        : `Prediction generated — ${cluster?.label} · Risk ${record.riskScore}/100`,
      {
        description: record.lowConfidence
          ? "Manual investigation recommended."
          : `${record.predictedTime} · Confidence ${record.confidence}% · ${stationForCluster(record.clusterId)?.name}`,
      },
    );
  });
  ...
  const ranked = [...atmClusters].sort((a, b) => b.riskScore - a.riskScore).slice(0, 3);
  ...
  return (
    <div className="space-y-7">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button
          onClick={() => void navigate({ to: "/trinity/cases" })}
